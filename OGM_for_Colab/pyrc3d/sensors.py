...
class Camera(Sensor):
    ...
    def __init__(self, 
        ...
        self.TARGET_POS = configs['camera_target_pos']
    ...
    def setup(self) -> None:
        ...
        Raises:
            None.
        """
        self.CAMERA = p.createVisualShape(p.GEOM_BOX, halfExtents=[0.02, 0.05, 0.02], rgbaColor=[1, 0, 0, .7])
    ...
    def create_point_cloud(self) -> None:
        ...
        reconstructor = Reconstruction([self.WIDTH,self.HEIGHT],[self.FX,self.FY],[self.WIDTH/2,self.HEIGHT/2],[[1,0,0],[0,1,0],[0,0,1]],[0,0,0])
        image = self.get_camera_image_rgb()
        depth = self.get_camera_image_depth()

        # Sample the image and depth map to only take every 50th pixel
        image = image[::20,::20,:]
        depth = depth[::20,::20]
        reconstructor.add_image(image,depth)
        reconstructor.point_cloud()
        reconstructor.visualize()
        self.POINT_CLOUD = reconstructor.pcd_o3d
        # Find the number of points in the point cloud
        print("Num points:", len(self.POINT_CLOUD.points))
