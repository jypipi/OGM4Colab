...
########### This section to load and store the simulation variables ###########
...
def Grid2World(grid, gridMapSize, res=0.1):
    ...
    x = str((j * res) - (gridMapSize/2 * res) + res/2)
    y = str((i * res) - (gridMapSize/2 * res) + res/2)
...
class OGM():
    ...
    class Simulation():
        """
        A sub-class used to perform the simulation of environment and robot.

        Attributes:
            sim (Sim): Simulation of environment.
            ctrl_tim (int): Control frequency.
            lidar_time (int): Lidar frequency.
            debugging_time (float): Print frequency for debugging.
            car (Car): Simulation of robot.
        ...
        def __init__(self):
            ...
            self.ctrl_time = Timings(CTRL_FPS)
            self.lidar_time = Timings(LIDAR_FPS)
            self.debugging_time = Timings(DEBUGGING_FPS)
            ...
            self.car.place_car(self.sim.floor)

            # Initialize the keyboard controller
            self.controller = KeyboardController(CTRL_FPS)
        ...
        def getSensorData(self, sensor: str='lidar', common=False):
            rays_data, dists, hitpoints = self.car.get_sensor_data(
                    sensor = sensor,
                    common = common
                )
            
            return rays_data, dists, hitpoints
    ...
    def mapping(self) -> np.ndarray:
        ...
        with keyboard.Listener(on_press=on_press) as listener:
            ...
            while listener.running:
                ...
                rays_data, dists, hitPoints = self.sim.getSensorData(
                        sensor = 'lidar',
                        common = False
                    )
                
                # Update the car's pose and log odds at time t
                x, y, yaw = self.sim.car.get_state(to_array=False)
                pose = (x, y, yaw)
                self.log_t = self.update_log_odds(hitPoints, pose)

                if self.sim.print_frequency.update_time():
                    print("Car's pose [x, y, theta]:", pose)

                # Simulate LiDAR.
                if self.sim.lidar_time.update_time():
                    self.sim.car.simulate_sensor('lidar', rays_data)

                # Perform controls
                if self.sim.ctrl_time.update_time():
                    v, s = self.sim.controller.navigate(x, y, yaw*180/pi)

                    # Perform action (drive)
                    self.sim.car.act(v, s)
    ...
    def perceptedCells(self, xt, hitPoints):
        ...
        for i in range(hitPoints.shape[0]):
            ...
            if np.isnan(point[0]):
                ...
                theta_body = Decimal(str(self.sim.rayStartAng)) + (Decimal(str(self.sim.beta)) * Decimal(str(i)))
                # Convert to range: +- (RAY_END_ANG - RAY_START_ANG)/2 [positive: ccw]
                ray_robot_ang = Decimal(str(pi/2)) - theta_body

                x0 = Decimal(str(self.sim.Z_max)) * Decimal(str(cos(ray_robot_ang + Decimal(str(xt[2]))))) + Decimal(str(xt[0]))
                y0 = Decimal(str(self.sim.Z_max)) * Decimal(str(sin(ray_robot_ang + Decimal(str(xt[2]))))) + Decimal(str(xt[1]))
            ...
            ray_grids = self.bresenham(xtGridWorld[0], xtGridWorld[1], endGridWorld[0], endGridWorld[1])
    ...
    def update_log_odds(self, hitPoints, xt):
        ...
        for grid in rayConeGrids_World:
            ...
            self.log_t[grid_coord[0]][grid_coord[1]] += self.inv_sensor_model(xt, grid, Zt, self.sim.Z_max, measPhi) \
                                                - self.log_prior
    ...
    def saveGridMap(self, probGridMap, path: str=None):
        ...
        Parameters:
            probGridMap (numpy 2D array): Array of occupancy probabilities of all grids.
            path (str): path that indicates the directory and filename.
        
        Returns:
            text file (.txt): file that saves the array of occupancy probabilities.
        """
...
def main():
    ...
    # Set the parameters
    resolution, log_prior, save_grid_map = 0.1, 0.0, False
    ...
    if save_grid_map:
        ogm.saveGridMap(probGridMap, path=None)
