...
class KeyboardController():
    ...
    def __init__(
            ...
            onkey=lambda x: None,
            key_event_interval=lambda x: None
        ):
        ...
        let keyEventInterval;
        
        const startKeyEventInterval = () => {
            keyEventInterval = setInterval(() => {
            google.colab.kernel.invokeFunction('notebook.on_key_interval', {}, {});
            },''' \
        + str(1000 / ctrl_fps) +  \
        ''');  // Change the interval time (in milliseconds) as needed
        };
        
        const stopKeyEventInterval = () => {
            clearInterval(keyEventInterval);
        };

        document.addEventListener('keydown', (event) => {
            const key = event.key;
            google.colab.kernel.invokeFunction('notebook.on_key', [key, true], {});
        });

        document.addEventListener('keyup', (event) => {
            const key = event.key;
            google.colab.kernel.invokeFunction('notebook.on_key', [key, false], {});
        });

        startKeyEventInterval();
        };

        registerKeyboardEvents();
        '''

        output.register_callback('notebook.on_key', onkey)
        output.register_callback('notebook.on_key_interval', key_event_interval)
        output.eval_js(self.javaScript)
...
class GoogleColabSliderController():
    """
    Performs control of velocity and steering using sliders in google colab
    """
    def __init__(
            self, 
            ctrl_fps,
        ...
        self.velocity_slider = widgets.IntSlider(min=-50, max=50, value=0, description='velocity:')
        self.steering_slider = widgets.FloatSlider(min=-1, max=1, value=0, description='steering:')

        widgets.interactive(self.__update_velocity_steering, velocity=self.velocity_slider, steering=self.steering_slider)
    ...
    def __update_velocity_steering(self, velocity, steering):
